...
function [c, ceq] = constraints4(z)
    ...
    % Ensure that the gaps between turbines are at least 1.5*D (where D is the diameter of the rotor)
    % sort positions
    [positions_s, sortIdx] = sort(positions);
    radii_s = radii(sortIdx);
    for i = 2:length(positions)
        c = [c, positions_s(i-1) - positions_s(i) + 1.5*radii_s(i)]; % positions(i) - positions(i-1) >= 1.5*radii(i)
    end
    ...
    for i = 1:length(uniqueGroups)
            groupIndices = find(groups == uniqueGroups(i));
                ...
                % Ensure all turbines in the same group have the same radius to within 0.4999 meter
                for j = 1:length(groupIndices)
                    c = [c, radii(groupIndices(1)) - radii(groupIndices(j)) - 0.4999]; % radii(groupIndices(1)) <= radii(groupIndices(j))
                    c = [c, radii(groupIndices(j)) - radii(groupIndices(1)) - 0.4999]; % radii(groupIndices(j)) <= radii(groupIndices(1))
                end
    end
